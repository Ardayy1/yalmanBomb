...
while 1:
    ...
    [̲̅A̲̅][̲̅r̲̅][̲̅d̲̅][̲̅a̲̅] [̲̅Y̲̅][̲̅a̲̅][̲̅l̲̅][̲̅m̲̅][̲̅a̲̅][̲̅n̲̅]   
    
    Sms: {}           {}by {}@Ardaylmnnn\n  
    """.format(Fore.LIGHTWHITE_EX, len(servisler_sms), Style.RESET_ALL, Fore.LIGHTRED_EX))
    try:
        menu = (input(Fore.LIGHTMAGENTA_EX + " 1- Basda sms göndersin (Normal)\n\n 2- Buna sakın basma (Turbo)\n\n 3- Çıkış\n\n" + Fore.LIGHTYELLOW_EX + " Seçim: "))
        if menu == "":
            continue
        menu = int(menu) 
    except ValueError:
        system("cls||clear")
        print(Fore.LIGHTRED_EX + "Hatalı giris yapma la bida.")
        sleep(3)
        continue
    if menu == 1:
        system("cls||clear")
        print(Fore.LIGHTYELLOW_EX + "Telefon numarasını başında '+90' olmadan yaz (Birden çoksa 'enter' tuşuna bas): "+ Fore.LIGHTGREEN_EX, end="")
        tel_no = input()
        tel_liste = []
        if tel_no == "":
            system("cls||clear")
            ...
            try:
                ...
                sonsuz = "(Bence yapma ama sonsuza kadar yapcaksan 'enter' a bas)"  
            except ValueError:
                system("cls||clear")
        ...
        system("cls||clear")
        try:
            print(Fore.LIGHTYELLOW_EX + "Mail adresi (Bilmiyorsanız 'enter' tuşuna basın çokda gerek yok): "+ Fore.LIGHTGREEN_EX, end="")
            mail = input()
        ...
        system("cls||clear")
        try:
            print(Fore.LIGHTYELLOW_EX + "Kaç saniye aralıkla göndermek istiyorsun 'bence 1 saniye iyidir ;)': "+ Fore.LIGHTGREEN_EX, end="")
            aralik = int(input())
        except ValueError:
            system("cls||clear")
        ...
        system("cls||clear")
        print(Fore.LIGHTYELLOW_EX + "Telefon numarasını başında '+90' olmadan yaz yoksa çalışmaz valla: "+ Fore.LIGHTGREEN_EX, end="")
        tel_no = input()
        try:
            int(tel_no)
            if len(tel_no) != 10:
        ...
        try:
            ...
            system("cls||clear")
            print("\nTamamla donuyoz menuye çok basma")
            sleep(2)
